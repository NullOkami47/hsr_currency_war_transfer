...
- worker 第一次收到 `Login expired. Please log in again` 時，會自動重新載入 Currency Wars 事件頁、等待八秒，再重試原請求一次；不會無限重試。
- 若 VM Chrome 本來就顯示帳號已登入，通常只需讓事件頁完整載入以刷新 API session，不必先登出再登入。
- 若自動重試後仍然失敗，請記下失敗時間與工作 ID，再啟動臨時登入模式。只有事件頁載入後仍持續回 `retcode -100` 時，才需要真正登出並重新登入。
