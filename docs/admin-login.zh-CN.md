...
生产环境及 Vercel 在 `CURRENCY_WAR_ADMIN_TOTP_SECRET` 缺失或无效时会安全地停止
管理员访问；本地开发仍可只使用密码。如确需在生产环境紧急停用 2FA，必须明确设置
`CURRENCY_WAR_ADMIN_ALLOW_PASSWORD_ONLY_PRODUCTION=1`。这会降级为高风险的单因素
验证；请记录原因、限制持续时间，并在恢复 TOTP 后立即删除该变量。
...
5. 连续失败后，同一客户端在单个 API instance 内可能暂时受限；默认是 10 分钟内
   最多 5 次失败。失败登录存储和已用代码防重放存储默认都只在当前进程内；除非把可注入的
   异步接口连接到共享状态，否则它们不能提供跨 Vercel instance 的全局保护。请停止重试，
   稍后再使用全新的代码登录。
