...
正式環境及 Vercel 在 `CURRENCY_WAR_ADMIN_TOTP_SECRET` 缺少或無效時會安全地停止
管理員存取；本地開發仍可只使用密碼。如確需在正式環境緊急停用 2FA，必須明確設定
`CURRENCY_WAR_ADMIN_ALLOW_PASSWORD_ONLY_PRODUCTION=1`。這會降級成高風險的單因素
驗證；請記錄原因、限制持續時間，並在恢復 TOTP 後立即移除該變數。
...
5. 連續失敗後，同一用戶端在單一 API instance 內可能暫時受限；預設為 10 分鐘內
   最多 5 次失敗。失敗登入儲存及已用代碼防重放儲存預設只在目前程序內；除非把可注入的
   非同步介面連接至共享狀態，否則它們不能提供跨 Vercel instance 的全域保護。請先停止
   重試，稍後再以全新的代碼登入。
