...
const messages = {
  "zh-Hant": {
    ...
    heroTitle: "從中國服找到攻略，帶到國際服。",
    heroCopy: "貼上分享連結，或用名稱與角色篩選候選攻略。確認陣容後，再交由管理員帳號建立國際服版本。",
    searchEyebrow: "搜尋條件",
...
Object.assign(messages["zh-Hant"], {
  ...
  heroCopy: "貼上分享連結，或用攻略名稱、作者、角色與羈絆篩選候選攻略。確認陣容後，再交由管理員帳號建立國際服版本。",
...
Object.assign(messages["zh-Hans"], {
  ...
  heroCopy: "粘贴分享链接，或用攻略名称、作者、角色与羁绊筛选候选攻略。确认阵容后，再交由管理员账号建立国际服版本。",
...
function applyTranslations() {
  ...
  if (elements["view-global-strategy"].dataset.url) {
    const strategyUrl = new URL(elements["view-global-strategy"].dataset.url);
    strategyUrl.searchParams.set("lang", globalLanguage);
    elements["view-global-strategy"].href = strategyUrl.toString();
  }
...
function resetTransferUi() {
  ...
  elements["view-global-strategy"].removeAttribute("href");
  delete elements["view-global-strategy"].dataset.url;
...
function renderTransferResult(data, candidate, pendingEntry = null) {
  ...
  if (data.globalUrl) {
    elements["view-global-strategy"].dataset.url = data.globalUrl;
    const strategyUrl = new URL(data.globalUrl);
    strategyUrl.searchParams.set("lang", state.locale === "zh-Hant" ? "zh-tw" : state.locale === "zh-Hans" ? "zh-cn" : "en-us");
    elements["view-global-strategy"].href = strategyUrl.toString();
  }
