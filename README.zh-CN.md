...
本项目提供攻略转移核心与由管理员管理的本地发布连接器，用于将《崩坏：星穹铁道》「货币战争」攻略从中国服转移至国际服。

用户可以通过中国服攻略 URL／ID 精确查找，或按攻略名称、作者、角色及羁绊筛选候选攻略。确认内容后，系统会交由管理员的发布 worker 创建或更新国际服版本，并返回攻略码与官方链接。

> [!IMPORTANT]
> 这是非官方工具。来源攻略文本会保留原文；国际服无法对应的项目会被跳过，并在结果中明确列出。
...
| 🛡️ 安全发布流程 | 通过国际服官方网页界面创建或编辑，不导出 Cookie、密码或浏览器存储数据 |
| ✅ 发布后验证 | 重新获取国际服攻略，标准化后比较玩法字段与完整标题前缀 |
...
- `mapping:adopt`：登记连接器创建前已存在的国际服攻略，供后续原地验证与更新。
- `transfer`：使用管理员专用 Chrome 配置文件执行完整的创建／更新／验证流程。

</details>

默认情况下，浏览器配置文件及来源至国际服的映射数据会存储在仓库之外：
...
网站会创建任务、通过 Vercel API 轮询进度，最后返回国际服攻略码与官方攻略链接。浏览器不会接触 worker 令牌或 HoYoLAB 会话。同一篇中国服攻略的进行中请求会共享一项任务；已完成的重复请求则会执行创建／更新／不变比较流程。
...
- 读取中国服标题已选的两个羁绊，将稳定特性 ID 映射至国际服 `zh-tw` 名称，再重建前缀，例如 `【6战技点4命运圣杯】` → `【6戰技點4命運聖杯】`。
- 保留其余中文标题及运营思路原文。
- 仅在标题不超过 60 个字符时加入 `｜作者名称`。
- 仅在运营思路不超过 800 个字符时加入 `来源：作者名称`。
- 忽略国际服无法使用的 ID，并在 `ignored` 中返回所有跳过项目。
- 如果已映射的国际服攻略仍与来源相同，返回现有攻略码。
- 如果来源已更改，原地编辑已映射的攻略。
- 如果不存在有效映射，创建新攻略。
- 返回成功前重新获取并比较已发布数据。
- 同时验证玩法字段与完整重建后的标题前缀。
...
<details>
<summary><strong>查看中国服及国际服端点</strong></summary>
