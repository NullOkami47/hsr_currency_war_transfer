...
在 Vercel 上，将 `CURRENCY_WAR_WORKER_URL` 及 `CURRENCY_WAR_WORKER_TOKEN` 设置为服务器端环境变量，即可将提交按钮连接至独立的已验证发布 worker。请在管理员电脑或持续运行的 VM 上，以相同令牌运行 `npm run worker`。网站会创建任务，通过 Vercel API 轮询进度，再返回全球服攻略码；浏览器不会接触 worker 令牌或 HoYoLAB 会话。同一篇中国服攻略的重复进行中请求会共用一项任务；已完成的重复请求则会执行现有的创建／更新／不变比较流程。
