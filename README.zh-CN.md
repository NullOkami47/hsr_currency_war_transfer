...
`npm run dev` 会在 `http://127.0.0.1:4173` 启动本地网站。网站支持以中国服 URL／ID 精确查找、以攻略名称及作者搜索、角色与羁绊（阵营／流派）多选 AND 筛选、候选攻略选择及提交转移。完成的提交会在此浏览器保存最多 50 条本地记录，方便用户再次取得全球服攻略码与官方链接。网站也提供浅色与深色模式；界面文字、角色名称及羁绊名称支持繁体中文、简体中文及英文，来源攻略文字则保持不变。
